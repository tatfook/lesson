            ...
            if(response.err == 0) {
                if( !Array.isArray(response.data) ){
                    ...
                    for(var i = 0; i < response.data.length; i++) {
                        ...
                        '    <div class="item-cover "><a href="' + (KEEPWORK_HOST + item.lessonUrl)+ '" target="_blank" class="title"><div class="cover" style="background-image: url('+ item.lessonCover +')"></div></a></div>' +
                        '    <div class="content">';
                        if( item.pkgs ){
                            itemStr += '<div class="package-title"><span>' + R.pkg + ': </span>';
                            for( var j = 0; j < item.pkgs.length;j++ ){
                                itemStr += '<a href="' + (KEEPWORK_HOST + item.pkgs[j].pkgUrl)+ '" target="_blank" class="title"> '+ item.pkgs[j].pkgTitle +'</a>'
                            }
                            itemStr += '</div>';
                        }
                        itemStr += '<div class="lesson-title"><a href="' + (KEEPWORK_HOST + item.lessonUrl)+ '" target="_blank" class="title">'+ R.lesson +' '+ item.lessonNo +'：<span>'+ item.lessonTitle +'</span></a></div>' +
