...
member.get = function(where, group, order, cn)
    local sql =
        "SELECT sn, username, portrait, coin, identity, presenter, firstInFlag, codeReadLine, codeWriteLine, commands, presenter, vipEndTime, UNIX_TIMESTAMP(vipEndTime) vipEndUnixTime, TIMESTAMPDIFF( DAY,NOW() , vipEndTime ) vipDay FROM member"
...
member.achieving = function(sn, cn)
    ...
    if (testrecordVo == nil) then
        ...
        local sql =
            "UPDATE member SET codeReadLine = codeReadLine + ?codeReadLine, codeWriteLine = codeWriteLine + ?codeWriteLine, commands = commands + ?commands WHERE username = ?username"
        return db.execute(
            sql,
            {
...
member.findOrInsertByName = function(username, portrait)
    local sql =
        "SELECT sn, (SELECT COUNT(1) FROM class WHERE teacher = member.username) teachedCount, username, portrait, coin, identity, presenter, firstInFlag, codeReadLine, codeWriteLine, commands, presenter, vipEndTime, UNIX_TIMESTAMP(vipEndTime) vipEndUnixTime, TIMESTAMPDIFF( DAY,NOW() , vipEndTime ) vipDay FROM member"
...
member.statis = function(where, group, order, cn)
    local sql =
        [[SELECT m.username, m.joinTime, m.codeReadLine, m.codeWriteLine, m.commands, m.presenter,
