...
class.taughtRecord = function(where, group, order, limit, cn)
    local sql =
        [[SELECT sn, classId, startTime, lessonUrl, lessonTitle, lessonNo, goals, lessonCover,
...
class.detail = function(where, group, order, cn)
    local sql =
        [[SELECT sn, classId, startTime, lessonUrl, teacher, lessonTitle, lessonNo, goals, startTime, finishTime,
