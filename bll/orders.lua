...
orders.detail = function(where, group, order, sn)
    local sql =
        [[SELECT sn, username, orderTime, payTime, amount, reallyPayAmount, lessAmount, couponID, endTime, goodsType, state FROM orders]]
...
orders.list = function(where, group, order, limit, cn)
    local sql =
        [[SELECT sn, username, orderTime, payTime, payWay, amount, reallyPayAmount, lessAmount, endTime, goodsType, state, TIMESTAMPDIFF( DAY,NOW() , endTime ) vipDay FROM orders]]
