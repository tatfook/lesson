...
subscribe.checkAddPackageByLessonUrl = function(username, lessonUrl, cn)
    local sql =
        "SELECT COUNT(1) FROM subscribe s LEFT JOIN package p ON s.`packageId` = p.`id` LEFT JOIN package2lesson pl ON p.`id` = pl.`packageId` WHERE s.`username` = ?username AND pl.`lessonUrl` = ?lessonUrl AND state = 1"
...
subscribe.addBatchByLessonUrl = function(username, lessonUrl, cn)
    local idList =
        db.queryAll(
        "SELECT p.id FROM package p LEFT JOIN package2lesson pl ON p.`id` = pl.packageId WHERE pl.lessonUrl = ?lessonUrl",
        {lessonUrl = lessonUrl},
        cn
    )
    local objs = {}
...
subscribe.state = function(where, group, order, cn)
    local sql =
        [[SELECT ( SELECT COUNT(1) FROM package2lesson pl WHERE p.`id` = pl.`packageId` ) AS lessonCount,
