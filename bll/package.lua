...
package.get = function(where, group, order, cn)
    local sql =
        "SELECT id, title, cover, skills, agesMin, agesMax, input, output, prerequisite, packageUrl FROM package"
...
package.list = function(where, group, order, limit, cn)
    local sql =
        [[SELECT p.*, sb.state subscribeState, ( SELECT COUNT(1) FROM package2lesson pl WHERE p.`id` = pl.`packageId` ) AS lessonCount,
