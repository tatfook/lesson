...
router:get(
	...
	function(req, res, next)
		local createOrder = function(user)
			...
			if (lastId) then
				res:redirect(
					"http://release.keepwork.com/wiki/pay?username=" ..
						username ..
							"&app_name=lessons&app_goods_id=1&price=" ..
								orderVo.amount ..
									"&additional={%22order_no%22:" .. lastId .. "}&redirect=http://lesson.keepwork.com/buy/result/" .. lastId
				)
			else
				-- 下订单失败
			end
