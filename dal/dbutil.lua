...
dbutil.upsert = function(tableName, obj, cn)
	...
	local sql =
		"insert into " ..
		tableName ..
			" (" .. keys .. ") values (" .. values .. ") ON DUPLICATE KEY UPDATE " .. dbutil._parsePlaceholderSet(obj)
	lastSqlString = sql
	dbutil.showSqlStr()
	return mysql:execNonQuery(sql, obj, cn)
end
...
dbutil.addBatch = function(tableName, fields, objs, cn)
	local sql =
		"insert into " .. tableName .. " (" .. dbutil.parseFields(fields) .. ") values " .. dbutil.parseObjects(objs)
	lastSqlString = sql
	dbutil.showSqlStr()
	return mysql:execNonQuery(sql, nil, cn)
end
